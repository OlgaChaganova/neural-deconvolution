...
from src.deconv.neural.rgdn.module_basic import ConvBlock
from src.deconv.neural.rgdn.utils import truncate_image
...
class OptimizerRGDN(Module):
    ...
    def __init__(
        ...
        if self.share_parameter:
            if(self.use_reg):
                self.CNNOptimizer = ConvBlock(is_rgb)
            if(self.use_grad_adj):
                self.GradAdj = ConvBlock(is_rgb)
            if(self.use_grad_scaler):
                self.GradScaler = ConvBlock(is_rgb)
